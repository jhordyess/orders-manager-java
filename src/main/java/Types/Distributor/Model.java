...
public class Model extends core.Models.DBAModel implements core.Models.Model {
  ...
  public JsonObject ReadAll(Server_Request srobj, String fly) {
    if ((new Ccte()).getReads().equals(fly)) {
      return super.LMDList(2, srobj, "dist");
    } else {
      MsgModel("code error");
      return null;
    }
  ...
  public String UpdateDate() {
    return super.updateD("distribuidor");
  }
  ...
  public String Whape(String who) {
    return super.LMDSimple(
        "SELECT IF(cel is null or cel='' or cel='-','',cel) as jh3 FROM `dist` WHERE nam='" + who + "';");
  }

  public String Face(String who) {
    return super.LMDSimple(
        "select if(cli.face is null or cli.face='' or cli.face='-','',cli.face) as jh3 from distribuidor as eve left join cliente as cli on eve.id_cliente=cli.id WHERE eve.nombre='"
            + who + "';");
  }

  public String Mail(String who) {
    return super.LMDSimple(
        "select if(cli.email is null or cli.email='' or cli.email='-','',cli.email) as jh3 from distribuidor as eve left join cliente as cli on eve.id_cliente=cli.id WHERE eve.nombre='"
            + who + "';");
  }
}
